...
int main(void) {
  ...
  // SetTargetFPS(GetCurrentRefreshRate());
  SetTargetFPS(60);
  //--------------------------------------------------------------------------------------

  startTime = GetTime();
  txtMovesCountWidth = MeasureText(txtMoves, txtMovesSize);
  GameInit(&game, 4);
  diff = (uint16_t *)calloc(game->grid->length, sizeof(uint16_t));
  gameOver = false;
  gridSize = game->grid->size;
  gridLength = game->grid->length;
  oldCells = (uint64_t *)calloc(gridLength, sizeof(uint64_t));
  tiles = (Rectangle *)calloc(gridLength, sizeof(Rectangle));
  movingTiles = (Rectangle *)calloc(gridLength, sizeof(Rectangle));
  tileSize =
...
void Update() {
  ...
  if (gameOver) {
    ...
    if (GridAnyCellAvailable(game->grid) || GameTileMatchesAvailable(game)) {
      int direction = -1;
      if (IsKeyPressed(KEY_LEFT) || currentGesture == GESTURE_SWIPE_LEFT) {
        direction = LEFT;
      } else if (IsKeyPressed(KEY_UP) || currentGesture == GESTURE_SWIPE_UP) {
        direction = UP;
      } else if (IsKeyPressed(KEY_RIGHT) || currentGesture == GESTURE_SWIPE_RIGHT) {
        direction = RIGHT;
      } else if (IsKeyPressed(KEY_DOWN) || currentGesture == GESTURE_SWIPE_DOWN) {
        direction = DOWN;
      }
      if (direction != -1) {
  ...
  const char *txtElapsed =
      TextFormat(txtElapsedFormat, hours, minutes, seconds);
  ...
  for (int i = 0; i < gridLength; ++i) // Draw all rectangles
  {
    ...
    if (moving) {
      ...
      for (int i = 0; i < gridLength; ++i) {
        ...
        while (MeasureText(txtTile, txtTileSize) > (int)actualTile.width - 4) {
          --txtTileSize;
        }
        txtTileWidth = MeasureText(txtTile, txtTileSize);
        int txtTileX =
            (int)(actualTile.x + (actualTile.width - (float)txtTileWidth) / 2);
        int txtTileY =
            (int)(actualTile.y + (actualTile.height - (float)txtTileSize) / 2);
